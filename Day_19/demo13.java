...
6. Accessing Elements:
   The head of the queue (highest priority element) can be
   accessed using the peek() method.

7. Removal of Elements:
   The highest priority element can be removed using the
   poll() method.

8. Comparator Support:
   Can be constructed with a custom comparator to define
   the order of elements.

9. Iterators:
   Provides an iterator, but the order of iteration is not
   guaranteed to match the priority order.

10. Performance:
    Insertion and removal operations have a time complexity of
    O(log n); accessing the head element is O(1).

11. Not a Blocking Queue:
    Does not block on operations; it is a non-blocking queue.
...
public class demo13 {
	...
	public static void main(String[] args) {
		...
		//Add tasks.offers with different priorities(real-world related)
		tasks.offer("String project report");  //High priority
		tasks.offer("Respond to client emails"); //Medium priority
		tasks.offer("Prepare for team meeting"); //Medium priority 
		tasks.offer("Updateproject documentation");  //Low priority
		tasks.offer("Schedule performnce review");  //High priority
	
	//Display the current tasks 
	System.out.println("Current Task :" + tasks);

	
	//Process tasks in order of priority
	System.out.println("Processing tasks in order of priority:");
	while(!tasks.isEmpty()) {
		System.out.println("Procesing :" +tasks.poll());
